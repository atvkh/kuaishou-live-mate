...
> **当前版本**：v1.0.7（v5.8 引擎） | **发布者**：atvkh | **License**：MIT
...
- **多模型自动回退**：视觉模型支持多模型优先级队列（`glm-4.6v-flash` → `glm-4.1v-thinking-flash` → `glm-4v-flash`），前一个 429 限流时自动切换下一个
- **拟人化评论生成**：根据弹幕和语音转录内容生成评论，带三种真人特征：15% 概率发水弹幕（"6"/"hhh"/"?"）、随机评论长度分布、30% 概率加语气词后缀
...
前往 [Releases 页面](https://github.com/atvkh/kuaishou-live-mate/releases) 下载最新版 `旁白_Setup_v1.0.7.exe`，双击安装即可。
...
6. 在浏览器中手动进入目标直播间
7. 程序自动开始工作：采集弹幕、转录语音、识别画面、生成并发送评论
